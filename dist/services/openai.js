...
import { aiReviewResponseSchema, aiTaskBuilderResponseSchema, } from "../shared/index.js";
...
- plagiarism_or_low_effort_signals (optional string array)`;
    ...
    const completion = await openai.chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: "You are a task submission reviewer. Respond only with valid JSON.",
            },
...
- difficulty_estimate (easy | medium | hard)`;
    ...
    const completion = await openai.chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: "You are a task marketplace expert. Respond only with valid JSON.",
            },
            { role: "user", content: prompt },
        ],
        response_format: { type: "json_object" },
...
export const generateDisputeSummary = async (params) => {
    ...
    const completion = await openai.chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: "Summarize this task dispute neutrally in 2-3 sentences for mediation.",
            },
            {
                role: "user",
...
export const scoreWorkerSkills = async (params) => {
    ...
    const { data: existing } = await supabase
        ...
        .single();
    if (existing) {
        const newScore = Math.min(100, Number(existing.score) * 0.7 + params.qualityScore * 0.3);
        await supabase
            .from("skill_scores")
            .update({
            score: newScore,
    ...
    const { data: profile } = await supabase
        ...
        .single();
    if (profile) {
        const trustBoost = params.qualityScore >= 90 ? 2 : params.qualityScore >= 70 ? 1 : -1;
        await supabase
            .from("profiles")
            .update({
            trust_score: Math.min(100, Math.max(0, Number(profile.trust_score) + trustBoost)),
        })
            .eq("id", params.profileId);
    }
};
